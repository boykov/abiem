...
subroutine set_dp4d_ptr(name,val)
  ...
  if (name .eq. "cache_bessel_jn") cache_bessel_jn => val
...
subroutine set_dp2d_ptr(name,val)
  ...
  if (name .eq. "quadsingular") quadsingular => val
...
subroutine set_i_ptr(name,val)
  ...
  if (name .eq. "dim_intG") dim_intG = val
end subroutine set_i_ptr

subroutine set_l_ptr(name,val)
  character(128) :: name
  logical, intent(in), target :: val
  if (name .eq. "use_int_neighbors_p") use_int_neighbors_p = val
  if (name .eq. "qbx_gauss6") qbx_gauss6 = val
  if (name .eq. "matrixa6_p") matrixa6_p = val
  if (name .eq. "gauss6") gauss6 = val
  if (name .eq. "qbx") qbx = val
end subroutine set_l_ptr
...
subroutine set_dc3d_ptr(name,val)
  character(128) :: name
  double complex, intent(in), target :: val(:,:,:)
  if (name .eq. "valG_y") valG_y => val
  if (name .eq. "intG_x") intG_x => val
  if (name .eq. "jacobian") jacobian => val
  if (name .eq. "farr") farr => val
  if (name .eq. "cache_phi") cache_phi => val
...
subroutine set_dc2d_ptr(name,val)
  character(128) :: name
  double complex, intent(in), target :: val(:,:)
  if (name .eq. "int_neighbors2") int_neighbors2 => val
  if (name .eq. "int_neighbors1") int_neighbors1 => val
