...
contains
  ...
  double precision function fAre(x,i)
    ...
    ! fAre = real(A_s(node_coordinates(i,:),x,dsqrt(2.0D0)*sigma(i),k_wave)*phi(x,i,hval**2))*hval**2/4
  ...
  double precision function fAim(x,i)
    ...
    integer, intent(in) :: i
    double precision, intent(in), dimension(:) :: x
    fAim = aimag(A_s(node_coordinates(i,:),x,dsqrt(2.0D0)*sigma(i),k_wave)*phi(x,i,hval**2))
  end function fAim
